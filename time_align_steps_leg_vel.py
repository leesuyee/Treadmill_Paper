...
def time_align_steps_leg_vel(total_flies, data): 
    ...
    R1_vel = np.empty(( total_flies, 1500 ))
    R2_vel = np.empty(( total_flies, 1500 ))
    R3_vel = np.empty(( total_flies, 1500 ))
    L1_vel = np.empty(( total_flies, 1500 ))
    L2_vel = np.empty(( total_flies, 1500 ))
    L3_vel = np.empty(( total_flies, 1500 )) # make empty num_flies x 1500 frame array
    ...
    for video in range( 0, len( data ) ): 
        ...
        for fly in range(0, num_flies): # iterate through video/fly and load in stored variables for repackaging 
            ...
            leg_vel = global_variables[ 6 + fly_num ] 
    ...
    #                 int_h_angle = h_angle.astype(int) 
                ...
                for leg in range (0, 6): 
                    if np.isnan(leg_vel).all() == True: 
                        pass

                    else: 
                        for idx in range(1, len(int_frame_idx)): 
                            if leg == 0: 
                                R1_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[0][ idx - 1  ]
                            if leg == 1: 
                                R2_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[1][ idx - 1  ]
                            if leg == 2: 
                                R3_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[2][ idx - 1  ]
                            if leg == 3: 
                                L1_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[3][ idx - 1  ]
                            if leg == 4: 
                                L2_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[4][ idx - 1  ]
                            if leg == 5: 
                                L3_vel[ fly_counter, int(int_frame_idx[idx])] = leg_vel[5][ idx - 1  ]
        ...
        leg_velocity = [ ] 
        leg_velocity.append((R1_vel, R2_vel, R3_vel, L1_vel, L2_vel, L3_vel))
